...
void adxl345_init(int fd)
{
	...
	usleep(11000); //wait 11.1ms
	//	wiringPiI2CWriteReg8(fd, 0x2e, 0x00);

	//Take 100 data point sample -> average 코드
	//샘플링 대신 0으로 초기값 설정
	wiringPiI2CWriteReg8(fd, 0x1e, 0x00);
	wiringPiI2CWriteReg8(fd, 0x1f, 0x00);
	wiringPiI2CWriteReg8(fd, 0x20, 0x00);
	
	//The DUR register : Disables the tap/double tap functions
	...
	//the window register
	//disable the double tap funciton
	wiringPiI2CWriteReg8(fd, 0x23, 0x00);

	//the Thresh_act register
